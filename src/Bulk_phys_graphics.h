...
void printEntity(entity* e, SDL_Surface* target, Uint32 color, int x, int y){
	if (e->useAnim){//If entity uses animation	
		...
		aacircleColor(target, b->position.x + x, b->position.y + y, b->radius, color);//Prints a circle
	}
		...
		aapolygonColor(target, px, py, 4, color);
	}
}
